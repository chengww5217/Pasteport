...
```
$ claude "このスクリーンショットの問題は？" /tmp/pasteport/9f2c1a4b7e0d3856/clipboard.png
                                          └── 貼り付けキーを押して現れたもの
```
...
クリップボードから取り出した画像は、拡張機能自身の global storage — つまり VS Code のプロファイル内 — に
一時保存されます。`/tmp` ではありません。共有された Linux マシンでは、他の利用者がそれを読めてしまうか、
予測できる名前でシンボリックリンクを仕込めてしまうからです。`Pasteport: Diagnose` が正確なパスを表示します。
...
`pasteport.remoteDir` のスコープはマシンです。ユーザー単位・マシン単位では設定できますが、ワークスペース
単位では設定できません。値はあなたのターミナルに書き込まれるので、リポジトリに決めさせるわけにはいきません。
