...
在 VS Code **遠端視窗**的終端機裡按下貼上鍵，本機剪貼簿裡的圖片或檔案就會傳到遠端主機 ——
接著**遠端路徑**寫進提示字元，命令列 AI 助手可以直接讀取。
...
在本機視窗裡一切照舊：按鍵會直接傳給終端機，所以這個快速鍵綁定留在任何視窗都不礙事。
...
傳輸走 VS Code 既有的連線：使用 `workspace.fs`，URI 直接借用視窗本身的，scheme 和 authority 也隨之
繼承。因此所有後端走的是同一條程式碼路徑，沒有需要按後端維護的分支邏輯。
...
正確，就不傳輸任何位元組，直接插入既有路徑。第二次貼上同一張截圖，只花一次往返。
...
按鈕的通知裡。這個預估值來自你連線上實測的吞吐量，因此門檻會自動適應，而不是猜一個位元組數。也可以
隨時執行 `Pasteport: Cancel Transfer` 命令。

取消會在檔案之間生效，而不是在檔案中途 —— `workspace.fs.writeFile` 沒有取消點，所以已經在傳輸的大檔案
會先傳完。
...
`remoteDir` 下的其他內容只會數一遍、不會動，所以即使把設定指向共用目錄，清理也不會誤傷別人的檔案。
...
如果哪裡不對，先執行 `Pasteport: Diagnose` —— 它會把一次成功貼上所依賴的每個條件都回報出來，附上它
再回報問題，會好處理得多。
...
| `pasteport.bracketedPaste`      | `false`  | 以括號貼上標記（`ESC[200~` … `ESC[201~`）包住插入內容 |

`pasteport.remoteDir` 是使用者層級設定，跟其他使用者設定一樣，可以被您開啟的工作區覆寫。它的值會直接
寫進您的終端機，所以複製來的儲存庫也能決定您的貼上會落到哪裡——請只在您信任的工作區裡貼上。`~` 不會展開 ——
`workspace.fs` 不解析它，那樣只會建立一個名稱就叫`~` 的目錄。
...
擴充功能不能在遠端側執行命令 —— 它運行在本機擴充功能主機裡。它能用的只有 `workspace.fs`，而每次讀取都由
遠端伺服器處理程序處理，所以 `/proc/self/environ` 就是那個處理程序自己的環境。它又是從你的登入環境啟動的，
因此它的 `TMPDIR` 正是主機實際設定的值。解析順序：
...
接著檔案會落在所選目錄下的 `pasteport` 子目錄中，寫入與清理也只涉及這個子目錄。每個主機只會記錄一次
偵測結果，`Pasteport: Diagnose` 也會回報，所以貼上去了哪裡從來不用猜。明確設定該值則完全跳過偵測。

有兩件事它解決不了。在與他人共用的遠端主機上，`TMPDIR` 通常就是 `/tmp`，於是 `/tmp/pasteport` 屬於第一個
貼上的人，之後的使用者會遇到權限錯誤 —— 請把 `pasteport.remoteDir` 設到你自己的位置，例如家目錄底下。
另外，偵測假定遠端是 POSIX 系統：目前支援的每個後端都是 Linux 或 macOS；Windows 遠端用的是 Windows
路徑風格，這套邏輯完全不認識。

關於 `quoting`：使用情境是會把輸入當作純文字接收的 TUI 助手，在那裡引號會被當成路徑的一部分，悄悄把
路徑弄壞。所以 `auto` 目前是原樣插入路徑。如果你主要是貼給會解析這一行的 shell，請選 `shell`。

關於 `bracketedPaste`：理解括號貼上的 TUI 會把包住的內容當作一次貼上而不是按鍵 —— 不自動縮排、不把其中
的換行立刻當成命令執行。如果你的助手介面會把插入的路徑弄壞，可以開啟它。預設關閉，因為各 TUI 的相容性
尚未驗證。
...
改變。如果你在自己的設定裡寫了 `commandsToSkipShell` 陣列，它會取代預設值 —— 此時擴充功能會提議幫你
補上這一項，只提示一次。
...
| 用戶端  | 讀取器                                      | 狀態                                                      |
| ------- | ------------------------------------------- | --------------------------------------------------------- |
| macOS   | `osascript`（JXA）呼叫 AppKit               | 已在實機驗證：截圖、只帶 TIFF 格式的複製內容、Finder 檔案 |
